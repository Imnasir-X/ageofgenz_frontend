 ...
 const showBookmarkToast = useCallback(
  ...
  useEffect(() => {
    ...
    const updateSticky = () => {
      ...
      const midpoint = offsetTop + height * 0.5;
      const triggerPoint = scrollTop + window.innerHeight * 0.4;
      const shouldLock = triggerPoint >= midpoint;
      setSidebarLocked((prev) => (prev === shouldLock ? prev : shouldLock));
    };
    const handleScroll = () => {
      if (frame) return;
  ...
  const estimatedReadTime = useMemo(() => {
    ...
    const imageCount = Array.isArray(article.content_blocks)
      ? article.content_blocks.filter((block) => block?.type === 'image').length
      : (article.content?.match(/<img/gi) || []).length;
    const baseMinutes = wordCount / wordsPerMinute;
    const imageMinutes = (imageCount * 12) / 60; // +12 seconds per image
    return Math.max(1, Math.ceil(baseMinutes + imageMinutes));
  }, [article, wordCount]);
  ...
  if (loading && !article) {
    return (
      <div className="min-h-screen bg-slate-50">
        <div className="fixed top-0 left-0 w-full h-[3px] bg-gray-200/80 z-50">
          <div className="h-full bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.4)] transition-all duration-300" style={{ width: '0%' }}></div>
        </div>
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      {showProgressBar && (
        <div className="fixed top-0 left-0 z-50 h-[3px] w-full bg-gray-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
          <div 
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 md:px-12 md:py-12 lg:px-16 lg:py-16">
        <div className="space-y-12 xl:flex xl:gap-12 xl:space-y-0">
                  ...
                  <div className="flex shrink-0 items-center gap-2 sm:gap-3">
                    ...
                    <div className="relative" ref={shareMenuRef}>
                      <button
                        ...
                        ref={shareTriggerRef}
                        className="inline-flex items-center gap-1 rounded-full border border-gray-300 bg-transparent px-2 py-1 text-[0.65rem] font-semibold text-gray-600 transform transition-colors transition-transform duration-150 hover:scale-105 hover:border-orange-200 hover:text-orange-600 hover:bg-orange-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-2 focus-visible:ring-offset-white sm:gap-1.5 sm:px-2.5 sm:py-1 sm:text-[0.7rem] md:gap-2 md:px-3 md:py-1.5 md:text-xs"
                        aria-haspopup="true"
          ...
          <aside className="mt-12 xl:mt-0 xl:w-1/3" aria-label="Related articles">
            <div className={`${sidebarLocked ? 'xl:sticky xl:top-24' : 'xl:static'} space-y-8 pb-8`}>
              <DonationPlaceholder />
