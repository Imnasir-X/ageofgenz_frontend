...
const AI: React.FC = () => {
  ...
  // Define a mapping of category names to colors - matching other pages format
  ...
  // Skeleton Loader Component for AI Articles
  const AIArticleSkeleton = () => (
  ...
  const fetchAiArticles = async () => {
    ...
    setError(null);
    try {
      console.log('🤖 Fetching AI articles...');
      
      // Use the category filtering API - same as other pages
      const response = await getArticlesByCategory('ai');
      console.log('AI API Response:', response.data);
      
      const fetchedArticles = response.data.results || [];
      console.log('Fetched AI Articles:', fetchedArticles);
      ...
      setError('Could not load AI articles. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <section className="mb-8 text-center">
          ...
          <p className="text-gray-600 text-base max-w-2xl mx-auto">
            Exploring artificial intelligence, machine learning, and the future of technology through a Gen Z perspective.
          </p>
        </section>
        ...
        <div className="bg-gray-900 rounded border border-gray-800 p-6 shadow-lg mx-auto">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <AIArticleSkeleton key={`ai-skeleton-${i}`} />
              ))}
            </div>
          ) : error ? (
            <ErrorWithRetry error={error} onRetry={fetchAiArticles} />
          ) : aiArticles.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-400 mb-4">No AI articles available yet.</p>
              <p className="text-gray-500 text-sm">
                Articles may need to be assigned to the AI category in the admin panel.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {aiArticles.map((article) => {
                console.log('Rendering AI Article:', article);
                ...
                const categoryName = article.category?.name || article.category_name || 'AI & Tech';
                const formattedCategoryName = formatCategoryName(categoryName);
                const categoryColor = categoryColors[formattedCategoryName] || categoryColors['AI & Tech'] || 'bg-blue-600';
