...
const Culture: React.FC = () => {
  ...
  // Skeleton Loader Component for Culture Articles
  const CultureArticleSkeleton = () => (
  ...
  const fetchCultureArticles = async () => {
    ...
    try {
      ...
      setCultureArticles(formattedArticles);
      setError(null);
    } catch (err: any) {
      console.error('Culture Fetch Error:', err);
      setError('Could not load culture articles. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <section className="mb-8 text-center">
          ...
          <p className="text-gray-600 text-base max-w-2xl mx-auto">
            Art, music, fashion, and the trends defining Gen Z. This is where culture gets loud and proud.
          </p>
        </section>
        ...
        <div className="bg-gray-900 rounded border border-gray-800 p-6 shadow-lg mx-auto">
          <h2 className="text-xl font-bold text-white mb-4 pb-2 border-b-2 border-orange-500">
            Latest Cultural Hits
          </h2>

          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <CultureArticleSkeleton key={`culture-skeleton-${i}`} />
              ))}
            </div>
          ) : error ? (
            <ErrorWithRetry error={error} onRetry={fetchCultureArticles} />
          ) : cultureArticles.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-400 mb-4">No culture articles available yet.</p>
              <p className="text-gray-500 text-sm">
                Articles may need to be assigned to the Culture category in the admin panel.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {cultureArticles.map((article) => {
                console.log('Rendering Culture Article:', article);
                ...
                const categoryName = article.category?.name || article.category_name || 'Culture';
                const formattedCategoryName = formatCategoryName(categoryName);
                const categoryColor = categoryColors[formattedCategoryName] || 'bg-yellow-600';
