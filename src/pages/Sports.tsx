...
const Sports: React.FC = () => {
  ...
  // Skeleton Loader Component for Sports Articles
  const SportsArticleSkeleton = () => (
  ...
  const fetchSportsArticles = async () => {
    ...
    try {
      ...
      const response = await getArticlesByCategory('sports');
      console.log('Sports API Response:', response.data);
      
      const fetchedArticles = response.data.results || [];
      console.log('Fetched Sports Articles:', fetchedArticles);
      ...
      setError('Could not load sports articles. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <section className="mb-8 text-center">
          ...
          <p className="text-gray-600 text-base max-w-2xl mx-auto">
            From game-changing plays to athlete activism—covering sports that matter to the next generation.
          </p>
        </section>
        ...
        <div className="bg-gray-900 rounded border border-gray-800 p-6 shadow-lg mx-auto">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <SportsArticleSkeleton key={`sports-skeleton-${i}`} />
              ))}
            </div>
          ) : error ? (
            <ErrorWithRetry error={error} onRetry={fetchSportsArticles} />
          ) : sportsArticles.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-400 mb-4">No sports articles available yet.</p>
              <p className="text-gray-500 text-sm">
                Articles may need to be assigned to the Sports category in the admin panel.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {sportsArticles.map((article) => {
                console.log('Rendering Sports Article:', article);
                ...
                // Get category color
                const categoryName = article.category?.name || article.category_name || 'Sports';
                const categoryColor = categoryColors[categoryName] || 'bg-emerald-600';
