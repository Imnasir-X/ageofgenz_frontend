...
import { Facebook, Mail, Linkedin, Copy, MessageCircle, Share } from 'lucide-react';
...
const SocialShare: React.FC<SocialShareProps> = ({
  ...
  // Babylon Bee style social options - comprehensive list
  const socialOptions = [
    {
      name: 'X',
      icon: <XLogo />,
  ...
  const primaryOptions = socialOptions.slice(0, 4);
  const secondaryOptions = socialOptions.slice(4);
  ...
  return (
    <div className={className}>
        ...
        {primaryOptions.map((option) => (
          option.external ? (
            <a
              ...
              className={`${option.bgColor} ${option.textColor} p-3 rounded-lg text-center transition-all hover:scale-105 group`}
            >
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1">
            <input
              ...
              value={url}
              readOnly
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-50 text-gray-700 font-mono"
      ...
      {/* Babylon Bee Style Footer Text */}
      <div className="mt-4 pt-4 border-t border-gray-200">
        <p className="text-xs text-gray-500 text-center">
          Help spread independent journalism • Share with friends who need to know
        </p>
      </div>
    </div>
  );
