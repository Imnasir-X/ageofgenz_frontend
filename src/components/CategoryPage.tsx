...
const CategoryPage: React.FC<Props> = ({
  ...
  const categoryIcon = useMemo(() => {
    ...
    return renderIcon('h-10 w-10 text-orange-200 sm:h-14 sm:w-14');
  }, [slug]);
  ...
  const searchHint = useMemo(() => {
    ...
    return 'Searching loaded articles only.';
  }, [searchQuery]);

  const effectiveEmptyMessage = useMemo(() => {
    const trimmedQuery = searchQuery.trim();
    if (trimmedQuery && filteredArticles.length === 0 && articles.length > 0) {
      return `No articles found matching \"${trimmedQuery}\".`;
    }
    return emptyMessage;
  }, [searchQuery, filteredArticles.length, articles.length, emptyMessage]);
    ...
    <ErrorBoundary>
      <div className="relative min-h-screen bg-slate-950 pb-16">
        ...
        <div className="relative mx-auto max-w-6xl space-y-10 px-4 pt-14 sm:px-6 lg:px-8">
          <header className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900 p-5 shadow-md sm:p-8">
              ...
              <div className="flex flex-1 flex-col gap-3">
                ...
                <div className="mt-1 flex flex-col gap-2">
                  <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
                    ...
                    <h1 className="bg-gradient-to-br from-orange-200 via-orange-400 to-orange-600 bg-clip-text text-3xl font-black tracking-tight text-transparent sm:text-4xl">
                      {title}
                    </h1>
                  </div>
                  <p className="max-w-2xl text-sm text-white/90 sm:text-lg">{description}</p>
                </div>
              </div>
            </div>
          </header>
